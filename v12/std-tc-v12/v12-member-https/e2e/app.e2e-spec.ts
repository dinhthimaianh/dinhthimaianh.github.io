...
describe('Starting tests for v12-member-https', function() {
  ...
  it('website title should be v12-member-https', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('v12-member-https');
    })
  });
  ...
  it('navbar-brand should be v12-member-https',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('v12-member-https');
    });
  });
